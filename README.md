...
![Live proxies](https://img.shields.io/badge/live%20proxies-702-22c55e?style=flat-square)
...
![Updated](https://img.shields.io/badge/updated-21%20Sep%202026%2005%3A35%20UTC-64748b?style=flat-square)
...
Right now you get **702 working proxies** in **71 countries**. Half of
...
| --- | ---: | --- |
| [`all.txt`](all.txt) | 702 | You want everything, `ip:port` on each line |
| [`all.csv`](all.csv) | 702 | You want to filter by country, speed, or uptime |
| [`all.json`](all.json) | 702 | Your code would rather not parse CSV |
| [`protocol/http/http.txt`](protocol/http/http.txt) | 351 | Your tool asks for an HTTP proxy. Start here |
...
| [`protocol/https/https.txt`](protocol/https/https.txt) | 389 | You need to load `https://` pages through the proxy |
| [`anonymity/elite/`](anonymity/elite) | 510 | The site must not see that you use a proxy |

A note on the HTTPS file. Many HTTP proxies can only forward plain `http://` pages. Loading an
`https://` page needs a different trick, called CONNECT, and only 389 of our proxies
...
The average proxy in this list has **76.3% uptime**. So if you load 100
...
You get 702 working proxies, updated every 5 minutes. Check the `anonymity` column
...
**Last updated:** 2026-09-21 05:35 UTC. This page and every file are generated
