...
![Live proxies](https://img.shields.io/badge/live%20proxies-409-22c55e?style=flat-square)
![Countries](https://img.shields.io/badge/countries-49-3b82f6?style=flat-square)
![Median speed](https://img.shields.io/badge/median%20speed-1,007%20ms-a855f7?style=flat-square)
![Updated](https://img.shields.io/badge/updated-28%20Aug%202026%2016%3A50%20UTC-64748b?style=flat-square)
...
Right now you get **409 working proxies** in **49 countries**. Half of
them answer in under **1,007 ms**.
...
| --- | ---: | --- |
| [`all.txt`](all.txt) | 409 | You want everything, `ip:port` on each line |
| [`all.csv`](all.csv) | 409 | You want to filter by country, speed, or uptime |
| [`all.json`](all.json) | 409 | Your code would rather not parse CSV |
| [`protocol/http/http.txt`](protocol/http/http.txt) | 184 | Your tool asks for an HTTP proxy. Start here |
| [`protocol/socks5/socks5.txt`](protocol/socks5/socks5.txt) | 125 | You need SOCKS5, which also carries non-web traffic |
| [`protocol/socks4/socks4.txt`](protocol/socks4/socks4.txt) | 100 | Your tool only speaks the older SOCKS4 |
| [`protocol/https/https.txt`](protocol/https/https.txt) | 155 | You need to load `https://` pages through the proxy |
| [`anonymity/elite/`](anonymity/elite) | 329 | The site must not see that you use a proxy |

A note on the HTTPS file. Many HTTP proxies can only forward plain `http://` pages. Loading an
`https://` page needs a different trick, called CONNECT, and only 155 of our proxies
...
countries/
  ...
  ...one folder per country, 49 right now
...
The average proxy in this list has **92.8% uptime**. So if you load 100
addresses from `all.txt` and try them all at once, expect about **93** to answer.
...
You get 409 working proxies, updated every 5 minutes. Check the `anonymity` column
...
**Last updated:** 2026-08-28 16:50 UTC. This page and every file are generated
