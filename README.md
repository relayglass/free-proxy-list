...
![Live proxies](https://img.shields.io/badge/live%20proxies-453-22c55e?style=flat-square)
![Countries](https://img.shields.io/badge/countries-65-3b82f6?style=flat-square)
![Median speed](https://img.shields.io/badge/median%20speed-1,114%20ms-a855f7?style=flat-square)
![Updated](https://img.shields.io/badge/updated-22%20Sep%202026%2015%3A15%20UTC-64748b?style=flat-square)
...
Right now you get **453 working proxies** in **65 countries**. Half of
them answer in under **1,114 ms**.
...
| --- | ---: | --- |
| [`all.txt`](all.txt) | 453 | You want everything, `ip:port` on each line |
| [`all.csv`](all.csv) | 453 | You want to filter by country, speed, or uptime |
| [`all.json`](all.json) | 453 | Your code would rather not parse CSV |
...
| [`protocol/socks4/socks4.txt`](protocol/socks4/socks4.txt) | 167 | Your tool only speaks the older SOCKS4 |
| [`protocol/https/https.txt`](protocol/https/https.txt) | 194 | You need to load `https://` pages through the proxy |
| [`anonymity/elite/`](anonymity/elite) | 370 | The site must not see that you use a proxy |

A note on the HTTPS file. Many HTTP proxies can only forward plain `http://` pages. Loading an
`https://` page needs a different trick, called CONNECT, and only 194 of our proxies
...
countries/
  ...
  ...one folder per country, 65 right now
...
The average proxy in this list has **84.3% uptime**. So if you load 100
...
You get 453 working proxies, updated every 5 minutes. Check the `anonymity` column
...
**Last updated:** 2026-09-22 15:15 UTC. This page and every file are generated
