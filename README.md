...
![Live proxies](https://img.shields.io/badge/live%20proxies-739-22c55e?style=flat-square)
![Countries](https://img.shields.io/badge/countries-84-3b82f6?style=flat-square)
![Median speed](https://img.shields.io/badge/median%20speed-871%20ms-a855f7?style=flat-square)
![Updated](https://img.shields.io/badge/updated-21%20Sep%202026%2004%3A20%20UTC-64748b?style=flat-square)
...
Right now you get **739 working proxies** in **84 countries**. Half of
...
| --- | ---: | --- |
| [`all.txt`](all.txt) | 739 | You want everything, `ip:port` on each line |
| [`all.csv`](all.csv) | 739 | You want to filter by country, speed, or uptime |
| [`all.json`](all.json) | 739 | Your code would rather not parse CSV |
| [`protocol/http/http.txt`](protocol/http/http.txt) | 350 | Your tool asks for an HTTP proxy. Start here |
...
| [`protocol/https/https.txt`](protocol/https/https.txt) | 398 | You need to load `https://` pages through the proxy |
| [`anonymity/elite/`](anonymity/elite) | 574 | The site must not see that you use a proxy |

A note on the HTTPS file. Many HTTP proxies can only forward plain `http://` pages. Loading an
`https://` page needs a different trick, called CONNECT, and only 398 of our proxies
...
countries/
  ...
  ...one folder per country, 84 right now
...
The average proxy in this list has **76.4% uptime**. So if you load 100
addresses from `all.txt` and try them all at once, expect about **76** to answer.
...
You get 739 working proxies, updated every 5 minutes. Check the `anonymity` column
...
**Last updated:** 2026-09-21 04:20 UTC. This page and every file are generated
